...
export default function DashboardHome() {
  // Handle empty ChartData by converting the data format to meet ChartData requirements
  const formatChartData = (data: any[]) => {
    return data.map(item => ({
      name: item.name,
      value: item.cases ? item.cases : item.value
    }));
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-bold">National Overview</h2>
        <p className="text-gray-500">Countrywide crime statistics and performance metrics</p>
      </div>
        ...
        <Card>
          ...
          <CardContent className="h-80">
            <StatisticsChart
              ...
              type="line"
              data={formatChartData(monthlyCasesData)}
              dataKeys={["cases"]}
              colors={["#3b82f6"]}
            />
          </CardContent>
